...
class BufferParser {
    ...
    /**
     * Reads a variable length wide string (UTF-16) from the buffer.
     *
     * @param {number} [length = 1] The number of characters to read
     * @returns {string | string[]}
     */
    wstring(length = 1) {
        let bytes = this.uint8(length * 2);
    ...
    /**
     * Reads a null terminated wide string (UTF-16) from the buffer.
     *
     * @returns {string}
     */
    wstring0() {
        ...
        while (!this.eof()) {
            ...
            result += Buffer.from([high, low]).toString('utf16le');
        }
